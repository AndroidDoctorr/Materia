...
public class AdvancedKilnRecipe implements Recipe<Container> {
    ...
    /**
     * True only when this recipe asks for coal coke in the fuel slot.
     * Bellows uses {@link #requiresBellows}; coal or charcoal qualify unless {@code requires_coke_fuel} is set.
     */
    ...
    public static class Serializer implements RecipeSerializer<AdvancedKilnRecipe> {
        ...
        public AdvancedKilnRecipe fromJson(ResourceLocation recipeId, JsonObject json) {
            ...
            // - some JSONs used "ingredient" instead of "ingredient1"
            // - some JSONs used an empty object {} for "ingredient2" to mean "no second ingredient"
            var ingredient1El = json.has("ingredient1") ? json.get("ingredient1") : json.get("ingredient");
            Ingredient ingredient1 = ingredient1El == null ? Ingredient.EMPTY : Ingredient.fromJson(ingredient1El);

            Ingredient ingredient2 = Ingredient.EMPTY;
            if (json.has("ingredient2")) {
                var ingredient2El = json.get("ingredient2");
                if (ingredient2El != null && !ingredient2El.isJsonNull()) {
                    if (ingredient2El.isJsonObject() && ingredient2El.getAsJsonObject().size() == 0) {
                        ingredient2 = Ingredient.EMPTY;
                    } else {
                        ingredient2 = Ingredient.fromJson(ingredient2El);
                    }
                }
            }
