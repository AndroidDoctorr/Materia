...
public class CokeOvenMenu extends AbstractContainerMenu {
    ...
    public CokeOvenMenu(int windowId, Inventory inv, FriendlyByteBuf extraData) {
        this(windowId, inv, inv.player.level.getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(4));
    }

    public CokeOvenMenu(int windowId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.COKE_OVEN_MENU.get(), windowId);
        checkContainerSize(inv, 3);
        blockEntity = (CokeOvenBlockEntity) entity;
    ...
    public ItemStack quickMoveStack(Player playerIn, int index) {
        ...
        if (index < VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT) {
            // From player inventory -> try fuel then input (both same filter)
            if (isCokable(sourceStack)) {
