...
public class WildGrapeVineFeature extends Feature<NoneFeatureConfiguration> {
    
    public WildGrapeVineFeature(com.mojang.serialization.Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }
        ...
        for (Direction direction : directions) {
            if (random.nextFloat() < 0.8f) { // 80% chance per direction - increased for better success rate
                ...
                if (level.getBlockState(vinePos).isAir()) {
                    ...
                    BlockState vineState = ModBlocks.WILD_GRAPE_VINE.get().defaultBlockState()
                    ...
                    if (random.nextFloat() < 0.7f) { // 70% chance to hang down
                        int hangLength = random.nextInt(4) + 1; // 1-4 blocks
                        for (int i = 1; i <= hangLength; i++) {
                            BlockPos hangPos = vinePos.below(i);
                            if (level.getBlockState(hangPos).isAir()) {
                                BlockState hangVineState = ModBlocks.WILD_GRAPE_VINE.get().defaultBlockState()
                                        .setValue(getPropertyForDirection(direction), true);
