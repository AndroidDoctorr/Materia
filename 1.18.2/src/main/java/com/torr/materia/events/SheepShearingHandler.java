    ...
    public static void onEntityInteract(PlayerInteractEvent.EntityInteract event) {
        ...
        if (!event.getWorld().isClientSide) {
            ...
            if (clumpItem != null) {
                ...
                int count = baseMin + event.getWorld().random.nextInt((baseMax - baseMin) + 1);
                
                for (int i = 0; i < count; i++) {
                    ItemStack clumpStack = new ItemStack(clumpItem);
                    ItemEntity itemEntity = new ItemEntity(event.getWorld(), 
                        sheep.getX(), sheep.getY() + 1, sheep.getZ(), clumpStack);
                    itemEntity.setDeltaMovement(
                        event.getWorld().random.nextGaussian() * 0.05,
                        0.2,
                        event.getWorld().random.nextGaussian() * 0.05
                    );
            ...
            // Damage the shears
            heldItem.hurtAndBreak(1, event.getPlayer(), (player) -> {
