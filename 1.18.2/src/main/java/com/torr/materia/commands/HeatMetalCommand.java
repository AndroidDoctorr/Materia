        ...
        if (context.getSource().getEntity() instanceof Player player) {
            ...
            if (!heldItem.isEmpty()) {
                heldItem.getCapability(HotMetalCapability.HOT_METAL_CAPABILITY).ifPresent(hotCapability -> {
                    ...
                    context.getSource().sendSuccess(new TextComponent("Heated " + heldItem.getDisplayName().getString() + 
                ...
                if (!heldItem.getCapability(HotMetalCapability.HOT_METAL_CAPABILITY).isPresent()) {
                    ...
                    context.getSource().sendFailure(new TextComponent("Item cannot be heated! Item: " + heldItem.getItem().toString() + ", In tag: " + inTag));
                }
            } else {
                context.getSource().sendFailure(new TextComponent("Hold an item to heat it!"));
            }
        }
        return 1;
    }
