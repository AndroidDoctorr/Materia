...
public class CartCoverModel extends EntityModel<CartEntity> {
    ...
    private static final float COVER_WIDTH = CartEntity.WIDTH + COVER_OVERHANG * 2.0F; // 1.25
    private static final float COVER_HEIGHT = 1.25F;
    private static final float COVER_LENGTH = CartEntity.LENGTH; // 2.0

    private static final float COVER_W = COVER_WIDTH * U; // 20.0
    private static final float COVER_H = COVER_HEIGHT * U; // 20.0
    private static final float COVER_L = COVER_LENGTH * U; // 40.0
    private static final float HALF_COVER_W = COVER_W * 0.5F; // 10.0
    private static final float HALF_COVER_L = COVER_L * 0.5F; // 20.0
    private static final float SHELL = 0.01F;

    private static final float H = CartEntity.HEIGHT * U; // 32.0
    private static final float FLOOR_H = H * CartEntity.FLOOR_HEIGHT_FRACTION; // 16.0  
    private static final float WALL_H = H - FLOOR_H; // 16.0
    private static final float WHEEL_D = CartEntity.WHEEL_RADIUS * 2.0F * U; // 32.0
    private static final float BODY_BASE = WHEEL_D * 0.5F; // 16.0e
    private static final float COVER_Y = BODY_BASE + FLOOR_H + WALL_H; // 48.0
