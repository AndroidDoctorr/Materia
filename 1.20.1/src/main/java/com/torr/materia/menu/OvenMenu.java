...
public class OvenMenu extends AbstractContainerMenu {
    ...
    public OvenMenu(int windowId, Inventory inv, FriendlyByteBuf extraData) {
        this(windowId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(4));
    }

    public OvenMenu(int windowId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.OVEN_MENU.get(), windowId);
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(handler -> {
            ...
            // Output slot (slot 2)
            this.addSlot(new SlotItemHandler(handler, 2, 108, 18));
        });
    ...
    public ItemStack quickMoveStack(Player playerIn, int index) {
        ...
        if (index < VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT) {
            ...
            if (FuelHelper.isOvenFuel(sourceStack)) {
