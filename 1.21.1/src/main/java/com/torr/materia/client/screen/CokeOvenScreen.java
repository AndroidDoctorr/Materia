...
public class CokeOvenScreen extends AbstractContainerScreen<CokeOvenMenu> {
    private static final ResourceLocation TEXTURE =
            ResourceLocation.fromNamespaceAndPath(materia.MOD_ID, "textures/gui/coke_oven.png");

    public CokeOvenScreen(CokeOvenMenu menu, Inventory inventory, Component component) {
    ...
    private void renderProgressArrow(GuiGraphics graphics, int x, int y) {
        if (menu.isCrafting()) {
            graphics.blit(TEXTURE, x + 81, y + 18, 177, 0, menu.getScaledProgress(), 16);
        }
    }
