...
public class InstrumentItem extends Item {
    ...
    private boolean shouldStartNow(ItemStack stack, Level level) {
        ...
        if (!data.contains(QUEUED_START_TIME_KEY)) {
            return false;
        }

        long queuedTime = data.copyTag().getLong(QUEUED_START_TIME_KEY);
    ...
    private boolean shouldPlaySample(ItemStack stack, Level level) {
        CustomData data = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY);
        if (!data.contains(ACTUAL_START_TIME_KEY)) {
            return false;
        }
        ...
        long startTime = data.copyTag().getLong(ACTUAL_START_TIME_KEY);
    ...
    private boolean isActive(ItemStack stack) {
        return stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).contains(ACTUAL_START_TIME_KEY);
    }
    ...
    private boolean isQueued(ItemStack stack) {
        CustomData data = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY);
        return data.contains(QUEUED_START_TIME_KEY) && !data.contains(ACTUAL_START_TIME_KEY);
    }
    ...
    public void onUseTick(Level level, LivingEntity livingEntity, ItemStack stack, int remainingUseTicks) {
        ...
        CustomData data = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY);
        if (data.contains(LAST_USE_TIME_KEY)) {
            long lastUse = data.copyTag().getLong(LAST_USE_TIME_KEY);
        ...
        if (shouldStartNow(stack, level)) {
            ...
            CustomData.update(DataComponents.CUSTOM_DATA, stack, tag -> {
                tag.remove(QUEUED_START_TIME_KEY);
                tag.putLong(ACTUAL_START_TIME_KEY, level.getGameTime());
            });
