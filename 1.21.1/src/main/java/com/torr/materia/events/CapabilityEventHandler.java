...
public class CapabilityEventHandler {
    ...
    public static void onAttachCapabilities(AttachCapabilitiesEvent<ItemStack> event) {
        ...
        if (canBeHeated(stack)) {
            event.addCapability(ResourceLocation.fromNamespaceAndPath(materia.MOD_ID, "hot_metal"), 
                new HotMetalCapability.Provider());
        }
        
        // Check if this item is tongs
        if (isTongs(stack)) {
            event.addCapability(ResourceLocation.fromNamespaceAndPath(materia.MOD_ID, "tongs"), 
                new TongsCapability.Provider());
        }
        
        // Check if this item is a steel pipe
        if (isSteelPipe(stack)) {
            event.addCapability(ResourceLocation.fromNamespaceAndPath(materia.MOD_ID, "glass_pipe"), 
    ...
    private static final TagKey<Item> HEATABLE_METALS = ItemTags.create(ResourceLocation.fromNamespaceAndPath(materia.MOD_ID, "heatable_metals"));
