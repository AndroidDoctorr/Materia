...
public class LoomCarpetCraftingHandler {
    ...
    private static Item resolveCarpetItem(String color) {
        // Prefer vanilla carpet if it exists (e.g., "red_carpet"), otherwise fall back to mod carpets (e.g., "taupe_carpet").
        Item vanilla = ForgeRegistries.ITEMS.getValue(ResourceLocation.fromNamespaceAndPath("minecraft", color + "_carpet"));
        if (vanilla != null && vanilla != Items.AIR) {
            return vanilla;
        }

        Item mod = ForgeRegistries.ITEMS.getValue(ResourceLocation.fromNamespaceAndPath(materia.MOD_ID, color + "_carpet"));
