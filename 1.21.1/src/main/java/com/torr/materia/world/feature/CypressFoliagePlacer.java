...
public class CypressFoliagePlacer extends FoliagePlacer {
    public static final MapCodec<CypressFoliagePlacer> CODEC = RecordCodecBuilder.mapCodec(instance ->
            instance.group(
                    IntProvider.CODEC.fieldOf("radius").forGetter(p -> p.radius),
                    IntProvider.CODEC.fieldOf("offset").forGetter(p -> p.offset),
                    com.mojang.serialization.Codec.intRange(1, 16).fieldOf("height").forGetter(p -> p.height)
    ...
    protected void createFoliage(LevelSimulatedReader level, FoliageSetter setter, RandomSource random, TreeConfiguration config, int maxFreeTreeHeight, FoliageAttachment attachment, int foliageHeight, int foliageRadius, int offset) {
        ...
        for (int layer = 0; layer < height; layer++) {
            ...
            placeLeavesRow(level, setter, random, config, base.above(layer), radius, 0, attachment.doubleTrunk());
        }
    }
