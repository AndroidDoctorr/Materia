...
public class HotMetalStackingUtils {
    ...
    public static ItemStack tryMergeStacks(ItemStack existing, ItemStack toAdd) {
        ...
        if (existing.getItem() == toAdd.getItem() && ItemStack.isSameItemSameComponents(existing, toAdd)) {
    ...
    public static ItemStack createCooledVersion(ItemStack hotStack) {
        ...
        CustomData data = hotStack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY);
        if (!data.isEmpty()) {
            var tag = data.copyTag();
            // Remove the hot metal capability data
            tag.remove("ForgeCaps");
            if (!tag.isEmpty()) {
                CustomData.set(DataComponents.CUSTOM_DATA, cooledStack, tag);
            }
        }
