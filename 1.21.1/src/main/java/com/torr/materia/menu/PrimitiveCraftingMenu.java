...
public class PrimitiveCraftingMenu extends AbstractContainerMenu {
    ...
    protected static void slotChangedCraftingGrid(AbstractContainerMenu menu, Level level, Player player, 
        ...
        if (!level.isClientSide) {
            ...
            java.util.List<ItemStack> items = new java.util.ArrayList<>(craftingContainer.getContainerSize());
            for (int i = 0; i < craftingContainer.getContainerSize(); i++) {
                items.add(craftingContainer.getItem(i));
            }
            CraftingInput input = CraftingInput.of(craftingContainer.getWidth(), craftingContainer.getHeight(), items);

            // Try to find a matching recipe
            for (net.minecraft.world.item.crafting.RecipeHolder<CraftingRecipe> holder : level.getRecipeManager().getRecipesFor(RecipeType.CRAFTING, input, level)) {
                CraftingRecipe craftingRecipe = holder.value();
                if (craftingRecipe.matches(input, level)) {
                    resultStack = craftingRecipe.assemble(input, level.registryAccess());
                    break;
                }
            }
