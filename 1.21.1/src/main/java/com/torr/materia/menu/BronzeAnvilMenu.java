...
public class BronzeAnvilMenu extends AbstractContainerMenu {
    ...
    public List<RecipeHolder<BronzeAnvilRecipe>> getAvailableRecipes() {
        ...
        return this.level.getRecipeManager().getAllRecipesFor(ModRecipes.BRONZE_ANVIL_TYPE.get()).stream()
                .filter(h -> h.value().matchesStacks(metal, tool0, tool1))
                .sorted(java.util.Comparator.comparing(h -> net.minecraftforge.registries.ForgeRegistries.ITEMS.getKey(h.value().getResultItem().getItem())))
                .collect(Collectors.toList());
    }

    @Override
    public boolean clickMenuButton(Player player, int id) {
        List<RecipeHolder<BronzeAnvilRecipe>> recipes = getAvailableRecipes();
        ...
        boolean hot = metal.getCapability(com.torr.materia.capability.HotMetalCapability.HOT_METAL_CAPABILITY).map(c->c.isHot()).orElse(false);
        if (!hot) return false;
        RecipeHolder<BronzeAnvilRecipe> holder = recipes.get(id);
        BronzeAnvilRecipe r = holder.value();
        ...
        String recipeName = holder.id().getPath();
        ...
        if (tool.hasCraftingRemainingItem()) {
            ...
            if (!container.isEmpty()) {
                ...
                if (container.getItem() instanceof com.torr.materia.item.StoneHammerItem) {
                    container.hurtAndBreak(2, player, net.minecraft.world.entity.EquipmentSlot.MAINHAND);
                }
            ...
            tool.hurtAndBreak(amount, player, net.minecraft.world.entity.EquipmentSlot.MAINHAND);
        }
    }
