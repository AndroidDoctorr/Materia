...
public class StoneAnvilMenu extends AbstractContainerMenu {
    ...
    public List<RecipeHolder<StoneAnvilRecipe>> getAvailableRecipes() {
        // Read slots from TE
        ...
        return this.level.getRecipeManager().getAllRecipesFor(ModRecipes.STONE_ANVIL_TYPE.get()).stream()
                .filter(h -> h.value().matchesStacks(metal, tool))
                .sorted(java.util.Comparator.comparing(h -> net.minecraftforge.registries.ForgeRegistries.ITEMS.getKey(h.value().getResultItem(this.level.registryAccess()).getItem())))
    ...
    public boolean clickMenuButton(Player player, int id) {
        List<RecipeHolder<StoneAnvilRecipe>> recipes = getAvailableRecipes();
        if (id < 0 || id >= recipes.size()) return false;

        RecipeHolder<StoneAnvilRecipe> holder = recipes.get(id);
        StoneAnvilRecipe recipe = holder.value();
